...
On first run, `pulse up` starts `pulse-server`, creates/reuses a local project and API key, and writes `~/.pulse/config.toml` without storing dashboard email/password credentials. `pulse dashboard` opens through a one-time local login URL.
