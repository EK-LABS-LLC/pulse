...
describe("local dashboard login", () => {
  ...
  test("creates a dashboard session from a local API key", async () => {
    ...
    const tokenResponse = await app.request(
      "http://127.0.0.1/dashboard/api/local-login-token",
      {
        method: "POST",
    ...
    const loginPath =
      new URL(token.login_url).pathname + new URL(token.login_url).search;
    ...
    const projectsResponse = await app.request(
      "http://127.0.0.1/dashboard/api/projects",
      {
        headers: {
    ...
    expect(projects.projects.some((row) => row.id === project.projectId)).toBe(
      true,
    );
  });
});
...
async function loadApp() {
  process.env.PULSE_HOME = mkdtempSync(
    join(tmpdir(), "pulse-local-login-test-"),
  );
  process.env.DASHBOARD_DIST_DIR = mkdtempSync(
    join(tmpdir(), "pulse-local-login-dist-"),
  );
  writeFileSync(
