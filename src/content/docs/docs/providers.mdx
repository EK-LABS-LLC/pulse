...
The span includes provider, model, token, cost, latency, status, session, trace name, prompt preview, and metadata attributes.
...
const client = observe(
  ...
  Provider.OpenAI,
  {
    ...
    sessionName: "Support conversation",
    traceName: "Answer customer question",
  }
);
