...
On first run, `pulse up` bootstraps local config without storing dashboard email/password credentials. `pulse dashboard` opens the local dashboard through a one-time login URL.
